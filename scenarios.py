...
def run_scenario(system, scenario, emissionsCap=None, costCap=None, save=True, solver="MOSEK", verbose=False):
    if scenario == "MinimizeCost":
        system.solve(objective="cost", solver=solver, verbose=verbose, max_iter=1000)
    elif scenario == "MinimizeEmissions":
        system.solve(objective="emissions", solver=solver, verbose=verbose, max_iter=1000)
    elif scenario == "MinimizeCostWithEmissionsCap":
        system.solve(objective="cost", emissionsCap=emissionsCap, solver=solver, verbose=verbose, max_iter=1000)
    elif scenario == "MinimizeEmissionsWithCostCap":
        system.solve(objective="emissions", costCap=costCap, solver=solver, verbose=verbose, max_iter=1000)
