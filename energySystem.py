...
class System:
    ...
    def _build_model(self, objective, emissionsCap=None, costCap=None):
        ...
        # set objective
        totalLoad = (self.powerLoad.sum() + self.heatLoad.sum())/1000
        if objective == 'cost':
            # minimize cost subject to total emissions cap
            self._objective = cp.Minimize(self.totalCost / 1e3)
            ...
            self._objective = cp.Minimize(self.totalEmissions / 1e3)
...
class HeatPump(Component):
    ...
    def __init__(self, n_timesteps=None, dt=None, COP=None, rampRate=None, capacityPrice=None, discRate=None, n_years=None):
        '''
        Inputs:
            - n_timesteps: number of time steps
            - dt: interval between time steps in hours
        ...
        # Parameters
        parameters = {'capacityPrice': cp.Parameter(nonneg=True, name='HeatPumpcapacityPrice'),
                      'COP': cp.Parameter(nonneg=True, name='HeatPumpCOP'),
                      'rampRate': cp.Parameter(nonneg=True, name='HeatPumprampRate')}
        parameterValues = {'capacityPrice': capacityPrice, 'COP': COP, 'rampRate': rampRate}
        # Variables
        ...
        # Constraints
        constraints = [heatOutput  <= capacity * dt , cp.abs(heatOutput[1:] - heatOutput[:-1]) / dt  <= rampRate * capacity * dt]
        # Consumption
